...
func update(id int, name string) {
	if _, ok := students[name]; ok {
		students[name] = Student{Id: id, Name: name}
		fmt.Printf("学生%s信息升级成功\n", name)
	} else {
		students[name] = Student{Id: id, Name: name}
		fmt.Printf("学生%s不存在，添加成功\n", name)
	}
}
func list() {
...
var (
	...
	adduser, deluser, updateuser, showlist, loadfile, savefile, flushdata bool
)

func init() {
	flag.BoolVar(&adduser, "add", false, "Add Student Notice,Usage:  add 1 XXX")
	flag.BoolVar(&deluser, "del", false, "Delete User ,Usage:  del XXX")
	flag.BoolVar(&updateuser, "update", false, "Update Student Notice,Usage:  update 1 XXX")
